...
Фактически это значит: до того как первый байт вашего HTTP‑запроса отправится на сервер, клиент и сервер должны договориться, что «мы оба на связи и готовы обмениваться данными». В реальных сетях это легко добавляет ~100 мс к первому запросу, особенно на мобильных сетях и при большом расстоянии до дата‑центра.
