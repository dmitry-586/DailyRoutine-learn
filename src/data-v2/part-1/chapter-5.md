...
| Срок жизни           | Настраивается         | До удаления  | До закрытия вкладки |
| Доступ из JavaScript | Да (если не HttpOnly) | Да           | Да                  |
| Защита от XSS        | HttpOnly флаг         | Нет          | Нет                 |
| Защита от CSRF       | SameSite              | Нет          | Нет                 |
...
- Same-Origin Policy — фундаментальный механизм безопасности браузера
- CORS настраивается на сервере, браузер лишь исполняет политику
- Preflight запросы нужны для сложных cross-origin запросов
- Cookies с HttpOnly защищают от XSS, SameSite защищает от CSRF
- Правильная настройка CORS и cookies критична для безопасности приложения
