...
⚠️ `100vw` может вызвать горизонтальный скролл из-за полосы прокрутки. Используйте `100%` вместо `100vw`.
...
- Container Queries решают проблему адаптивности компонентов
- CSS Variables — стандартный способ темизации и дизайн-токенов
- clamp(), min(), max() упрощают адаптивную типографику
- Grid с auto-fit/auto-fill создаёт адаптивные сетки автоматически
- Дизайн-токены через CSS Variables обеспечивают консистентность
- Современные единицы (rem, vw, fr) делают адаптив проще
