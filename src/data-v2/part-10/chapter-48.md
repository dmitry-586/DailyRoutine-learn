...
- Context API для передачи данных без prop drilling
- Проблема: все потребители ререндерятся при изменении
- Решение: разделение контекстов, мемоизация значений
- Подходит для редко меняющихся данных
- Не подходит для частых обновлений
- Альтернативы: Zustand, TanStack Query, Redux
- Разделение State и Dispatch уменьшает ререндеры
