# Глава 47. Context API: труба для доставки данных

Context API — это не замена Redux и не система управления состоянием. Это **труба для доставки данных** через дерево компонентов, минуя промежуточные уровни.

Представь водопровод: вода течёт по трубе от источника к крану. Context работает так же: данные текут от Provider к Consumer, минуя промежуточные компоненты. Но если в трубе изменится давление (значение контекста), **все краны (компоненты, использующие контекст) получат новое давление**, даже если им нужна только часть данных.

**Важно понимать:** Context — это способ доставки, а не система управления. Неправильное использование может привести к проблемам с производительностью.
...
## 47.2. Эффект домино: когда изменение темы ломает форму

Представь: у тебя есть контекст с данными пользователя и темой интерфейса. Пользователь переключает тему — и **все компоненты, использующие этот контекст, перерендериваются**, даже если им нужны только данные пользователя, а не тема.

Это **эффект домино**: изменение одного маленького значения (тема) заставляет перерендериваться всех подписчиков, даже тех, кому это изменение не нужно.
...
Изменение `theme` вызывает ререндер `UserProfile`, хотя он использует только `user`. Изменение `user` вызывает ререндер `ThemeSwitcher`, хотя он использует только `theme`. Это неэффективно и может привести к проблемам производительности.

**Решение: разделение контекстов**

Разделяй контексты по частоте обновления и области использования. Это предотвращает лишние ререндеры: каждый компонент подписывается только на нужный контекст.
