...
## 45.3. Cleanup функция: подчистка за собой
...
Представь: ты подписался на событие или запустил таймер. Если компонент размонтируется, а подписка останется — это утечка памяти. Cleanup функция «подчищает за собой», предотвращая такие проблемы.
