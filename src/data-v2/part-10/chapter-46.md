# Глава 46. Оптимизация рендеров: useMemo, useCallback, useRef

**Главное правило:** не оптимизируй то, что и так работает быстро. Преждевременная оптимизация усложняет код, добавляет накладные расходы и может не дать ожидаемого эффекта. Сначала пиши код без оптимизаций, затем **профилируй** и оптимизируй только то, что действительно медленно.
