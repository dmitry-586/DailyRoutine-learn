...
## 49.3. Стратегия размещения: изоляция виджетов

Главный вопрос: **где размещать Error Boundaries?** Ответ зависит от того, какую часть приложения ты хочешь защитить. Но главный принцип: **изолируй виджеты, чтобы падение одного не ломало всё приложение**.

Представь дашборд: виджет погоды, форма оплаты, график продаж. Если виджет погоды упадёт из-за ошибки API — это не должно ломать форму оплаты. Пользователь должен иметь возможность оплатить, даже если погода не загрузилась.
...
### Вокруг критичных компонентов: изоляция виджетов

Самый гранулярный подход — размещать Error Boundaries вокруг отдельных виджетов. Это стратегия **«вложенных предохранителей»**: если падает один виджет, всё приложение должно продолжать работать.

**Пример: изоляция виджета погоды**
...
function Dashboard() {
  return (
    <div>
      ...
      {/* Виджет погоды изолирован — его падение не сломает форму */}
      <ErrorBoundary fallback={<WeatherError />}>
        <WeatherWidget />
      </ErrorBoundary>
      
      {/* Форма оплаты изолирована — её падение не сломает график */}
      <ErrorBoundary fallback={<PaymentFormError />}>
        <PaymentForm />
      </ErrorBoundary>
      
      {/* График изолирован — его падение не сломает таблицу */}
...
Если виджет погоды упадёт из-за ошибки API, пользователь всё равно сможет оплатить заказ. Если форма оплаты упадёт, график продаж продолжит работать. Это **изоляция ошибок**: падение одного виджета не должно ломать всё приложение.

**Правило:** размещай Error Boundaries вокруг виджетов, которые могут упасть независимо друг от друга. Это гарантирует, что падение одного виджета не парализует всё приложение.
