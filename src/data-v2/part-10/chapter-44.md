...
- Virtual DOM — объектное представление DOM в памяти для эффективного обновления
- Reconciliation — процесс сравнения деревьев и определения минимальных изменений
- Рендер ≠ обновление DOM — компонент может рендериться без изменения DOM
- Keys критичны для эффективного обновления списков
- Индекс — плохой key, используй стабильные ID
- Побочные эффекты должны быть в useEffect, а не в теле компонента
- Fiber позволяет прерывать рендеринг для отзывчивости интерфейса
