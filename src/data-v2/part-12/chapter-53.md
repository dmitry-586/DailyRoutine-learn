...
- **staleTime** — как долго данные считаются актуальными (0 по умолчанию — данные устаревают мгновенно)
- **gcTime** — как долго хранить неиспользуемые данные в кэше (5 минут по умолчанию)
...
**Важно:** в TanStack Query v5 `cacheTime` переименован в **`gcTime`** (Garbage Collection Time). Это критически важно для собеседований.
...
**Важно:** в TanStack Query v5 поддерживается только **объектный формат** параметров. Старый синтаксис `useQuery(queryKey, queryFn, options)` больше не работает.
...
- **isPending** — первая загрузка (данных ещё нет в кэше) — **новое в v5, заменило `isLoading`**
...
**Важное различие:** `isPending` показывает скелетон при первом заходе (когда данных нет в кэше). `isFetching` показывает спиннер при обновлении уже загруженных данных. Это позволяет не прятать контент при фоновом обновлении.

**Breaking Change v5:** `isLoading` переименован в `isPending` для более точного описания состояния (данные могут быть в кэше, но запрос ещё выполняется).
...
**Пример:** `staleTime: 1000 * 60` // данные свежие 1 минуту

**По умолчанию:** `staleTime: 0` — данные устаревают мгновенно, но остаются в кеше для мгновенного отображения.

### gcTime — время хранения в кеше

**Breaking Change v5:** `cacheTime` переименован в **`gcTime`** (Garbage Collection Time).
...
**Пример:** `gcTime: 10 * 60 * 1000` // данные хранятся 10 минут после размонтирования

**По умолчанию:** `gcTime: 5 * 60 * 1000` (5 минут).
