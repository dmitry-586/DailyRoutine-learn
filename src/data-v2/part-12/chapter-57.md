...
- Server Components могут делать async запросы напрямую
- Fetch кэшируется по умолчанию, можно настроить через `revalidate`
- ISR для периодического обновления статического контента
- Streaming с Suspense для лучшего UX
- Image компонент для оптимизации изображений
- next/font для оптимизации шрифтов
- Code splitting происходит автоматически по маршрутам
