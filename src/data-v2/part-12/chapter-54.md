...
## 54.1. useMutation: изменение данных
...
- **mutate(data)** — запускает мутацию (fire-and-forget, не требует try/catch)
- **mutateAsync(data)** — возвращает Promise (для await, требует try/catch)
- **isPending** — мутация выполняется
- **isSuccess/isError** — результат мутации
- **reset()** — сброс состояния

**Рекомендация:** используйте `mutate` в большинстве случаев, так как он не требует ручного `try/catch` — ошибки обрабатываются через `onError`. `mutateAsync` нужен только когда требуется синхронная обработка результата.
...
**Упрощённый способ в v5:** можно использовать `variables` из хука мутации для доступа к данным мутации в `onMutate`:

```typescript
const updateUser = useMutation({
  mutationFn: updateUserApi,
  onMutate: async (newUser) => {
    // newUser доступен напрямую
...
## 54.5. Infinite Query (бесконечные списки)
...
Для предотвращения утечек памяти в v5 добавлен параметр **`maxPages`**, который ограничивает количество страниц в памяти:
...
useInfiniteQuery({
  ...
  maxPages: 10, // Максимум 10 страниц в памяти
})
```
...
## 54.6. Dependent Queries (зависимые запросы)
...
## 54.7. Глобальная обработка ошибок
