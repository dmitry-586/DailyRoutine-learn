# Глава 9. Рендеринг: DOM, CSSOM, layout, paint, composite, reflow и repaint
...
## 9.1. Парсинг HTML и построение DOM
...
- **async** — скачивается параллельно, выполняется сразу после загрузки (может прервать парсинг)
- **defer** — скачивается параллельно, выполняется после полного парсинга HTML (не прерывает парсинг)

---

## 9.2. CSSOM и блокирующие ресурсы
...
2. Если использовать сложные селекторы (div ul li span:first-child), CSSOM пересчёт становится дорогим. Каждое изменение структуры DOM заставляет браузер снова пробегать дерево стилей и решать, какие правила подходят под какой элемент.
...
## 9.3. Render Tree и вычисление Layout
...
- частичным (только изменённые элементы)
- полным (вся страница)
- рекурсивным (вложенные элементы)
...
## 9.4. Paint, Composite и GPU-ускорение
...
```javascript
// Читаем все значения сначала
const heights = elements.map(el => el.offsetHeight)

// Потом пишем все изменения
...
## 9.7. requestAnimationFrame и оптимизация анимаций
...
## 9.8. Почему важен Critical Rendering Path
...
### 9. Какие элементы не входят в Render Tree?
