...
- Типизация функций включает параметры и возвращаемое значение
- Опциональные параметры через `?`, параметры по умолчанию через `=`
- Rest параметры для переменного количества аргументов
- Перегрузки функций для разных сигнатур
- Типизация `this` через параметр `this: Type`
- Async функции возвращают `Promise<Type>`
- Типы функций для переиспользования сигнатур
