...
- `any` отключает проверку типов — использовать крайне редко
- `unknown` требует проверки типа — безопасная альтернатива any
- `never` для невозможных значений и exhaustive checks
- Union types (`|`) для ограничения набора значений
- Intersection types (`&`) для объединения типов
- Literal types для конкретных значений
- const assertions для узких типов и readonly
