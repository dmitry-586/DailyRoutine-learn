...
### Работа компилятора через АСД

Компилятор TypeScript работает через **Абстрактное Синтаксическое Дерево (АСД)**. Процесс состоит из трёх этапов:

1. **Парсинг** — компилятор преобразует исходный TypeScript код в Абстрактное Синтаксическое Дерево (AST). Это структурированное представление кода, где каждый узел соответствует синтаксической конструкции.

2. **Проверка типов** — на основе АСД выполняется статический анализ типов. Компилятор проверяет совместимость типов, ищет ошибки и предупреждения.

3. **Транспиляция** — АСД преобразуется в JavaScript код. Все аннотации типов и конструкции уровня типов полностью удаляются, остаётся только код уровня значений.

**Важно:** Типы в TypeScript **никогда не влияют на выполнение кода**. Они существуют только на этапе компиляции и используются исключительно для проверки корректности. После компиляции весь код уровня типов полностью исчезает.

### Разделение на уровни: код уровня типов и код уровня значений

TypeScript разделяет код на два уровня:

**Код уровня типов** (Type-level code):
- Существует только на этапе компиляции
- Полностью удаляется после компиляции
- Не влияет на runtime
- Примеры: `interface`, `type`, `:`, дженерики `<T>`

**Код уровня значений** (Value-level code):
- Остаётся в JavaScript после компиляции
- Выполняется в runtime
- Примеры: функции, переменные, классы, выражения

```typescript
// Код уровня типов (исчезает после компиляции)
interface User {
  name: string
}
...
function process<T>(value: T): T {
  // Код уровня значений (остаётся в JS)
  return value
}
```
...
```javascript
// Всё, что остаётся — только код уровня значений
...
TypeScript поддерживает **постепенную типизацию** (gradual typing):

- Можно типизировать код поэтапно
- Необязательно типизировать всё сразу
- Полезно при миграции JavaScript → TypeScript
- Можно использовать `any` для постепенного перехода

```typescript
// Начинаем с минимальной типизации
...
// Финальная версия с полной типизацией
function calculate<T extends number>(a: T, b: T): T {
  return (a + b) as T
}
```

Это позволяет мигрировать большие проекты постепенно, без необходимости переписывать весь код сразу.
...
TypeScript поддерживает стандартные типы JavaScript:
