...
⚠️ Не используйте стрелки как методы объектов, если нужен `this`.
...
- возвращает итератор
- используется для ленивых вычислений, асинхронных итераторов
...
|---------------|---------------------|---------------------|----------------|
| Hoisting | Полное | Только переменная | Только переменная |
| this | Собственный | Собственный | Из внешнего контекста |
| arguments | Есть | Есть | Нет |
| new | Можно | Можно | Нельзя |
| prototype | Есть | Есть | Нет |
| Имя в стеке | Да | Нет (или через NFE) | Нет |
...
- Function Declaration поднимается целиком, Function Expression — только переменная
- Стрелочные функции не имеют собственного `this` и `arguments`
- Стрелки нельзя использовать как конструкторы
- Выбор вида функции зависит от контекста использования
- Понимание различий критично для правильной работы с `this` и замыканиями
