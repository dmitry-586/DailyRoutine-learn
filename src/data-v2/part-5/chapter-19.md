...
**Важное исключение:** `typeof` для переменной в TDZ выбросит ошибку `ReferenceError`. Это исключение из правила, так как обычно `typeof` считается "безопасным" оператором, возвращающим `undefined` для необъявленных переменных. Но в TDZ даже `typeof` не работает.
