...
`exports` — это ссылка на `module.exports`. Нельзя переназначать `exports` напрямую.
...
- Когда определяется: на этапе сборки (статически)
- Tree-shaking: да
- Строгий режим: по умолчанию
- Топ-уровневый await: да (в некоторых контекстах)
- Циклические зависимости: обрабатываются корректно
- Где используется: браузеры, современный Node.js

**CommonJS:**

- Синтаксис: `require/module.exports`
- Когда определяется: во время выполнения (динамически)
- Tree-shaking: нет
- Строгий режим: нет
- Топ-уровневый await: нет
- Циклические зависимости: могут быть проблемными
- Где используется: старый Node.js, некоторые сборщики
