...
**Про JSON-способ (`JSON.parse(JSON.stringify(...))`)**: можно встретить в старом коде, но он ломает `Date`, теряет `undefined`/функции/`Symbol` и падает на циклических ссылках — лучше избегать.
